...
|---|---:|---:|---|
| `software-product-estimation` | 3 | 6 | Covered by router gates, size-duration separation, false-precision guard, and handoff output. |
...
| `proxy-component-estimation` | 3 | 5 | Covered by worksheet, S/M/L classes, new/modified/refactor status, NFR flags, and proxy calibration gate. |
| `function-point-estimation` | 4 | 7 | Covered by worksheet-only mode, ILF/EIF/EI/EO/EQ classification, navigation exclusion, workbook block, and AI human-review gate. |
| `estimation-to-plan` | 4 | 7 | Covered by range-only forecast, required assumptions, DoD/QA/UAT line items, Brooks's Law guard, and cross-team velocity block. |
| `estimation-review` | 4 | 7 | Covered by severity ordering, false-precision Critical finding, modified-code High finding, and source-boundary notes. |
| **Total** | **25** | **44** | **Pass — all requirements and scenarios mapped at concise static level.** |
